...
**_Identity_****_ _**refers to the core concept of what an Entity is. It’s represented by a *token* that usually has an externally visible network *identifier* such as a Domain Name or an IP Address.

**_Validation _**is the step where an identity is said to be true and correct — not a fake.

**_Authentication_****_ _**is the process by which a person proves that he is in control of an identifier, usually by means of authentication such as a password or private key. 
